...
플래티넘 이상의 데이터와 전체 데이터에 대해 CNN, 단순 합산 두 모델을 비교하였다. 모든 경우에서 CNN 모델이 단순 합산 모델보다 높은 정확도를 보여주었는데, 이는 CNN 모델이 게임 데이터의 복잡한 패턴을 더 잘 학습하고, 이를 바탕으로 더욱 정확한 승패 예측을 수행했음을 나타낸다.

단순 합산 모델은 플레이어의 개별 통계치를 합산하여 결과를 도출하는 방식으로, 모든 플레이어의 데이터가 동일한 방식으로 처리되며, 각 플레이어 간의 복잡한 상호작용을 고려하지 못한다. 반면에, CNN 모델은 합성곱 계층을 통해 플레이어 사이의 복잡한 상호작용을 반영하는 방식으로, 데이터의 지역적 특성을 파악하고 이를 바탕으로 복잡한 패턴을 학습한다.


## 3.3 소결
이번 프로젝트에서는 플래티넘 이상의 데이터와 전체 데이터에 대해 CNN 모델과 단순 합산 모델을 사용하여 성능을 비교하였다.

결과적으로, CNN 모델이 단순 합산 모델보다 더 높은 정확도를 보여주었습니다. 이는 CNN 모델이 데이터의 복잡한 패턴을 더 잘 학습하고, 이를 바탕으로 더욱 정확한 승패 예측을 수행했음을 나타냅니다. 특히, 각 플레이어 간의 복잡한 상호작용을 고려하는 합성곱 계층을 통해, CNN 모델은 데이터의 지역적 특성을 파악하고 복잡한 패턴을 학습하였습니다.

반면, 단순 합산 모델은 플레이어의 개별 통계치를 합산하여 결과를 도출하는 방식으로, 모든 플레이어의 데이터를 동일하게 처리하였습니다. 이로 인해 이 모델은 각 플레이어 간의 복잡한 상호작용을 고려하지 못하였습니다.

우리는 또한 주요 데이터 항목인 KDA, DPM, DPG, DPD, DTPM 등에 대한 추정과 분석을 통해 이들이 승패 예측에 어떻게 영향을 미치는지 분석하였습니다. 이러한 데이터 항목들은 플레이어의 전반적인 활약, 고딜 기여, 자원 활용, 생존 능력, 맵 컨트롤 등을 반영하며, 이들의 상호작용이 승패 예측에 큰 영향을 미치는 것으로 나타났습니다.

따라서 이번 프로젝트를 통해, 우리는 복잡한 게임 데이터를 처리하는 데 있어, 단순 합산 방식보다는 복잡한 패턴을 학습할 수 있는 모델이 더욱 효과적일 수 있음을 확인하였습니다. 이는 앞으로의 데이터 분석과 모델 개발에 있어 중요한 인사이트를 제공합니다.
...
따라서, 이 프로젝트를 통해 단순 합산 모델과 CNN 모델 각각의 장단점을 파악할 수 있었으며, 이는 더 나은 승패 예측 모델을 개발하는 데 참고할 수 있습니다. 이러한 결과는 복잡한 게임 데이터를 다루는 데 있어, 단순 합산 방식보다는 복잡한 패턴을 학습할 수 있는 모델이 더욱 효과적일 수 있음을 보여줍니다.

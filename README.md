...
플래티넘 이상의 데이터와 전체 데이터에 대해 두 모델의 성능을 비교해보면, 모든 경우에서 CNN 모델이 단순 합산 모델보다 높은 정확도를 보여주었다. 이는 CNN 모델이 게임 데이터의 패턴을 더 잘 학습하고, 이를 바탕으로 더욱 정확한 승패 예측을 수행할 수 있음을 보여준다. 

이 프로젝트에서는 단순 합산 모델과 CNN 모델, 두 가지 다른 방식을 통해 게임 승패를 예측하는 모델의 성능을 비교하였고, 두 모델의 성능 차이는 각각의 데이터를 다루는 방식과 학습 방법에서 나타난다.

단순 합산 모델은 플레이어의 개별 통계치를 합산하여 결과를 도출하는 방식으로, 모든 플레이어의 데이터가 동일한 방식으로 처리되며, 각 플레이어 간의 복잡한 상호작용을 고려하지 못한다. 반면, CNN 모델은 합성곱 계층을 통해 플레이어 사이의 복잡한 상호작용을 반영하는 방식으로, 데이터의 지역적 특성을 파악하고 이를 바탕으로 복잡한 패턴을 학습한다. 

이런 차이로 인해 CNN 모델은 단순 합산 모델과 비교하여 더욱 높은 성능을 보여주었다.

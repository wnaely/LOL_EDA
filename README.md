...
프로젝트를 통해 다량의 League of Legends 게임 데이터를 분석하고, 승패를 예측하는 모델을 개발하는 과정에서 데이터 과학 및 머신러닝 기술에 대한 이해와 경험이 증가했을 것입니다.
2. 게임 이해의 깊이:

LoL의 다양한 측면에 대한 데이터 분석을 통해 게임의 전략, 팀워크, 챔피언 간의 상호작용 등에 대한 깊은 이해를 얻을 수 있었을 것입니다. 이를 통해 게임의 복잡성을 이해하고, 플레이어들 간의 경쟁에서 중요한 영향을 미치는 요소들을 파악할 수 있게 되었을 것입니다.
3. Riot Games API 활용:

Riot Games API를 사용하여 게임 데이터를 효과적으로 수집하고 분석하는 경험을 쌓았을 것입니다. API를 통한 데이터 획득, 전처리, 모델 학습 등의 과정을 이해하고 활용할 수 있게 된 것이 중요한 스킬로 나타날 것입니다.
4. 통계 및 예측 모델 평가:

개발한 승패 예측 모델의 성능을 평가하고, 모델의 강점과 약점을 파악하며 모델을 향상시키는 데 필요한 통계적 평가 기술에 대한 이해가 높아질 것입니다.

...
Riot Games는 게임 자체의 재미뿐만 아니라 분석 가능한 다양한 게임 데이터를 무료로 제공하고 있다<sup>[[04]](https://developer.riotgames.com/)</sup>. 소환사 정보, 챔피언 정보, 게임 정보, 랭킹 및 리그 정보 등의 게임 데이터는 Riot Games의 API를 통해 접근할 수 있다. 데이터는 하루에 약 20만 건의 게임 데이터가 수집되며, 소정의 절차를 거쳐 쉽게 접근 할 수 있다. 해당 데이터를 통해 게임에 대한 통계적 분석과 예측 모델을 구축할 수 있다.
...
## 2.1 데이터 소스
이번 프로젝트에 활용할 데이터는 온라인 게임 코칭 전문기업인 더매치랩<a href="http://tml.kr/">(The Match LAB)</a>에서 가공한 LoL 게임 데이터를 바탕으로 한다. 
...
| 포지션 | 위치  |역할|
|:--------:|:---:|-----|
| 탑<br>(TOP) |   상단 라인   | 탑 라이너는 게임 맵의 상단에 위치하며, 주로 챔피언 간의 1대1 싸움에 특화되어 있다. 강인한 챔피언들이나 방어적인 챔피언들이 탑 라인에서 주로 플레이하며, 이를 통해 팀 전체의 균형을 유지한다. |
| 정글<br>(JUG) |  정글 지역, 전체 라인  | 정글러는 정글 지역에서 몬스터를 사냥하며 경험치와 골드를 획득한다. 다른 라인으로 이동하여 팀원들을 돕거나, 중요한 몬스터를 처치하여 팀 전체에게 이점을 제공하는 등 다양한 역할을 수행한다. |
| 미드<br>(MID) |  중앙 라인  | 미드 라이너는 맵의 중앙에 위치하며, 위치상으로 다른 라인을 지원하거나 중요한 목표물을 향해 빠르게 이동하는데 유리하다. 대체로 빠른 레벨업과 골드 획득이 가능하며, 이를 바탕으로 게임 초반부터 중반까지 강한 영향력을 발휘할 수 있다. |
| 원딜<br>(ADC) |  하단 라인<br>( BOTTOM )  | 원딜러는 주로 맵의 하단 라인에서 플레이한다. 주로 공격력이 높고, 공격 속도가 빠른 챔피언이 플레이하며, 게임 후반에 팀의 주요 피해 딜러 역할을 한다. 체력이 약해 보호가 필요하며, 골드를 획득해 빠르게 아이템을 완성하는 것이 중요하다.|
...
|:-------------:|:--------------:|-------------|
| win | 승패 여부 | 해당 데이터는 게임의 최종 결과를 나타낸다. 모델의 성능을 평가하는 데 사용되며, 이번 프로젝트의 최종 목표인 승패 예측에 있어 가장 중요한 데이터다. 모델이 이 데이터를 정확하게 예측하는 것이 이번 프로젝트의 완성도를 결정짓게 된다. |
...
| dpd | 데스 당 가한 데미지 | 플레이어가 죽은 횟수 당 얼마나 많은 데미지를 적에게 가했는지를 나타내는 지표다. 이는 플레이어의 공격 효율성을 나타낸다. 'dpd'가 높을수록 플레이어가 적에게 더 많은 피해를 줄 수 있음을 의미하므로, 게임에서의 우위를 점하는데 중요한 역할을 한다. |
| baron | 아군 바론 처치 수 | 바론을 처치하면 팀 전체에게 골드와 경험치를 제공하며, 일정 시간 동안 팀 전체의 공격력이 증가하는 등의 보너스를 부여한다. 이는 게임의 전환점이 될 수 있다. |
...
| turretKills | 포탑 킬 수 (막타) | 포탑을 파괴하면 골드와 맵 컨트롤 우위를 얻을 수 있으므로 중요한 전략적 목표이다. 포탑을 파괴함으로써 적진에 더 깊게 진입할 수 있는 길을 열어주며, 게임의 승리에 큰 기여를 한다. |
...
<table align="center">
  ...
  <tr align="center"><th>티어</th><th>단계</th><th>분포</th><th>합</th><th rowspan="4">골드 G</th><td>I</td><td>2.68%</td><td rowspan="4">19.26%</td></tr>
  <tr align="center"><th>챌린저 C</th><td>I</td><td>0.01%</td><td>0.01%</td><td>II</td><td>3.92%</td></tr>
  <tr align="center"><th>그랜드마스터 R</th><td>I</td><td>0.02%</td><td>0.02%</td><td>III</td><td>4.85%</td></tr>
  <tr align="center"><th>마스터 M</th><td>I</td><td>0.47%</td><td>0.47%</td><td>IV</td><td>7.81%</td></tr>
  <tr align="center"><th rowspan="4">다이아 D</th><td>I</td><td>0.39%</td><td rowspan="4">3.45%</td><th rowspan="4">실버 S</th><td>I</td><td>2.78%</td><td rowspan="4">19.04%</td>
  </tr>
  <tr align="center"><td>II</td><td>0.55%</td><td>II</td><td>4.01%</td></tr>
  <tr align="center"><td>III</td><td>0.69%</td><td>III</td><td>4.87%</td></tr>
  <tr align="center"><td>IV</td><td>1.82%</td><td>IV</td><td>7.38%</td></tr>
  <tr align="center"><th rowspan="4">에메랄드 E</th><td>I</td><td>1.59%</td><td rowspan="4">13.68%</td><th rowspan="4">브론즈 B</th><td>I</td><td>3.3%</td><td rowspan="4">19.93%</td></tr>
<tr align="center"><td>II</td><td>1.93%</td><td>II</td><td>4.5%</td></tr>
  <tr align="center"><td>III</td><td>3.21%</td><td>III</td><td>5.08%</td></tr>
  <tr align="center"><td>IV</td><td>6.95%</td><td>IV</td><td>7.05%</td></tr>
  <tr align="center"><th rowspan="4">플래티넘 P</th><td>I</td><td>2.15%</td><td rowspan="4">16.97%</td><th rowspan="4">아이언 I</th><td>I</td><td>3.11%</td><td rowspan="4">7.21%</td></tr>
  <tr align="center"><td>II</td><td>3.22%</td><td>II</td><td>2.54%</td></tr>
  <tr align="center"><td>III</td><td>4.21%</td><td>III</td><td>1.08%</td></tr>
  <tr align="center"><td>IV</td><td>7.39%</td><td>IV</td><td>0.48%</td></tr>
</table>
</details>
...
전체 게임 데이터의 티어별 분포를 살펴보면 '브론즈'부터 '에메랄드' 티어의 플레이어 데이터가 가장 많다. 반면, 챌린저와 그랜드마스터 티어의 플레이어 데이터 수는 상대적으로 매우 적은 것을 확인할 수 있는데, 아는 실제 LoL 게임의 티어 분포와 비슷하게 나타난다.
...
| 데이터 | 0825 | 0915 | 0917 |
|:--:|:----:|:----:|:----:|
| 플래티넘 ↑ | 44,962 | 28,209 | 25,208 |


플래티넘 티어 이상의 경기 데이터에서 'kda', 'dpd', 'dpm', 'dpg', 'dtpm'를 핵심 데이터로 속성으로 추출했다.
'kda'는 플레이어의 킬, 데스, 어시스트를 종합적으로 고려한 지표로, 플레이어의 전반적인 전투 성과를 나타낸다.
'dpd'는 죽은 횟수당 입힌 피해량을, 'dpm'은 분당 입힌 피해량을 나타내는 지표로, 플레이어의 공격 효율성을 측정한다.
'dpg', 'dtpm'은 각각 골드 당 가한 데미지와 분당 받은 데미지를 나타내는 지표로, 자원 활용 능력과 받은 피해량을 반영한다.
...
## 3.1 모델 개요
이 프로젝트에서는 단순 합산 방법과 CNN(합성곱 신경망) 두 가지 방법을 사용하였다.
...
마지막으로, Flatten을 통해 2차원의 특징 맵을 1차원으로 변환하고, 이를 바탕으로 Dense를 통해 분류 작업을 수행한다.
...
<table>
  ...
  <tr align="center"><td>전체</td><td>93.6%</td><td>94.1%</td></tr>
...
0825, 0915, 0917의 각 날짜별로 플래티넘 이상의 데이터와 전체 데이터에 대한 승패 예측 모델의 정확도를 나타내보면 위와 같다.

플래티넘 이상의 데이터를 보면, 모든 날짜에서 CNN을 활용한 승패 예측 모델이 단순 합산 모델의 정확도보다 더 높은 성능을 보여주고 있다. 특히 0825 데이터에서는 95.8%의 정확도를 보이며 가장 높은 성능을 나타냈다.

전체 데이터에서도 CNN 모델이 더 높은 정확도를 보였으며, 단순 합산 모델의 결과와 달리 모든 날짜에서 92% 이상의 높은 정확도를 유지했다.

---

## 3.3 소결

플래티넘 이상의 데이터와 전체 데이터에 대해 CNN, 단순 합산 두 모델을 시용하여 성능을 비교하였다. 

모든 경우에서 CNN 모델이 단순 합산 모델보다 높은 정확도를 보여주었는데, 이는 CNN 모델이 게임 데이터의 복잡한 패턴을 더 잘 학습하고, 이를 바탕으로 더욱 정확한 승패 예측을 수행했음을 나타낸다.
반면, 단순 합산 모델은 플레이어의 개별 통계치를 합산하여 결과를 도출하는 방식으로, 모든 플레이어의 데이터가 동일한 방식으로 처리되어 각 플레이어 간의 복잡한 상호작용을 고려하지 못했다. 

프로젝트에서는 KDA, DPD, DPG, DPM, DTPM 등을 핵심 데이터로 선정하고, 이들이 승패 예측에 어떻게 영향을 미치는지 분석하였다. 
KDA는 플레이어의 전반적인 전투 성과를, DPD와 DPM은 플레이어의 공격 효율성을 측정하고, DPG와 DTPM은 자원 활용 능력과 받은 피해량을 반영하는 역할을 했다.
해당 데이터는 전투와 관련된 지표로 서로 강한 연관성을 가지며, 이를 통해 게임 안에서의 플레이어 활동과 성과를 정확하게 파악할 수 있었고, 높은 정확도의 승패를 예측하는 모델을 구축할 수 있었다.
...
League of Legends 게임 데이터를 활용한 승패 예측 모델을 개발하는 프로젝트는 새로운 경험이었다.
이번 프로젝트는 단순히 승패를 예측하는 것이 아닌 각 지표들의 연관성을 직접 찾고 분석하여, 더 나은 모델과 결과를 내기 위해 더 깊이 있는 분석을 진행했다.

'CNN'과 '단순 합산 방식' 두 가지 다른 모델을 사용하여 분석하였으며, 단순 합산 방식으로 진행했을 때는 예측 정확도가 90%를 넘지 못하는 결과가 있어 아쉬웠지만, CNN을 활용하여 다시 모델링을 진행하였을 때는 전체적으로 정확도가 상승하여 모두 92% 이상의 높은 정확도를 보여주었다. 이를 통해, 복잡한 패턴을 학습할 수 있는 딥러닝 모델의 활용이 예측 성능 향상에 매우 중요하다는 것을 알게 되었다.

또한, 다양한 지표를 분석하고 연관성이 있는 지표들을 추출하는 과정의 중요성을 깨달았다. 다음 프로젝트에서는 전투 관련 지표 외에도 성장 관련 지표, 시야 및 유틸 관련 지표 등의 중요 지표를 분석하여 더욱 높은 성능의 모델을 개발하여 정확도를 높이는 것을 목표로 하고 있다.

프로젝트를 진행하는 부분에 있어 LoL 이라는 게임에 대한 깊은 이해가 필요하였기에, 처음에는 관련 지식이 부족해서 어려움을 겪었다.
하지만, LoL에 대한 관련 자료들을 찾아보며 학습하고, 직접 게임을 해보는 등의 노력을 통해 LoL에 대해 자세히 알아가는 과정을 거쳐 이를 극복하였다.
이러한 배경 지식은 데이터 분석과 모델링 작업에 하는데 있어 더 깊이 있는 분석을 가능하게 하였다. 

프로젝트를 진행하며 LoL이 왜 전 세계적으로 인기 있는 게임인지를 이해할 수 있었고, 롤의 다양한 전략적 요소와 플레이어 간의 협력이 필요한 게임 매커니즘이 많은 플레이어들을 끌어들이는 매력적인 요소임을 깨닫게 되었다.

결과적으로, 이번 프로젝트는 롤에 대한 깊이 있는 이해를 통해 데이터 분석과 모델링 능력을 향상시키는 데 큰 도움이 되었고, 이 경험은 향후 다양한 데이터 분석 프로젝트에 있어서도 도움이 될 것이라 생각한다.
